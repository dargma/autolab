#!/usr/bin/env python3
"""Exp-001: MNIST architecture sweep — parallel CPU training + latency check."""
...
def _write_live_row(result):
    """Callback: append one result row to sweep-live.csv immediately."""
...
class DepthwiseCNN(nn.Module):
    def __init__(self, channels, fc_layers):
        ...
        for i in range(len(channels) - 1):
            if i == 0:
                # first layer: standard conv (in_ch may be 1)
                convs.extend([
                    nn.Conv2d(channels[i], channels[i + 1], 3, padding=1),
...
def run_candidate(args):
    candidate, train_cfg, constraint = args
    name = candidate["name"]
    seed = train_cfg["seed"]
...
def main():
    ...
    print(f"🔍 MNIST Architecture Sweep — {len(candidates)} candidates, {n_workers} workers")
    print(f"   Constraint: ≤{constraint['max_latency_ms']}ms latency (batch_size={constraint['batch_size']})")
    print()

    args_list = [(c, train_cfg, constraint) for c in candidates]

    # Remove stale live CSV
    if LIVE_CSV.exists():
        LIVE_CSV.unlink()

    # Launch async workers — each result streams to sweep-live.csv immediately
